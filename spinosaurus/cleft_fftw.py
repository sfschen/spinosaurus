...
class CLEFT:
    ...
    def __init__(self, k, p, kIR=None, one_loop=True, shear=True, third_order=True, cutoff=10, jn=5, N = 2000, threads=None, extrap_min = -4, extrap_max = 2, import_wisdom=False, wisdom_file='wisdom.npy'):
    ...
    def p_integrals(self, k):
        ...
        for l in range(self.jn):
            ...
            bias_integrands[0,:] = 1. - 0.5 * ksq * ( (self.Xloop+self.Xlin_gt) + (self.Yloop+self.Ylin_gt)*mu2fac )\
                + kcu * shiftfac * (self.Vloop + self.Tloop * mu3fac)/6.\
                + ksq**2/8 * (self.Xlin_gt**2 + 2*self.Xlin_gt*self.Ylin_gt*mu2fac + self.Ylin_gt**2*mu4fac) # matter
            bias_integrands[1,:] = (-2 * k * (self.Ulin+self.U3)) * shiftfac - ksq*(self.X10 + self.Y10*mu2fac) \
                                    + k**3 * self.Ulin * (self.Xlin_gt * shiftfac + self.Ylin_gt * mu3fac * shiftfac) # b1
            bias_integrands[2,:] = self.corlin * (1 - 0.5*ksq*self.Xlin_gt - 0.5*ksq*self.Ylin_gt*mu2fac) - ksq*mu2fac*self.Ulin**2 - shiftfac*k*self.U11 # b1sq
